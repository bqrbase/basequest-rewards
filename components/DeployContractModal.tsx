...
export default function DeployContractModal({
  ...
  async function handleDeploy() {
    ...
    try {
      ...
      const response = await fetch("/api/quests/deploy-contract/complete", {
        method: "POST",
      ...
      if (response.ok && json.success && json.progress) {
        onQuestCompleted(json.progress);
        return;
      }

      // Do not hide Supabase errors — surface code/message/details/hint.
      console.error("[DeployContractModal] save failed — exact response:", json);
      const supabaseParts = [
        json.supabase?.code ? `code=${json.supabase.code}` : null,
        json.supabase?.message || json.error || null,
        json.supabase?.details ? `details=${json.supabase.details}` : null,
        json.supabase?.hint ? `hint=${json.supabase.hint}` : null,
      ].filter(Boolean);

      setErrorMessage(
        supabaseParts.length > 0
          ? supabaseParts.join(" | ")
          : "Contract deployed onchain, but saving progress failed.",
      );
    } catch (error) {
