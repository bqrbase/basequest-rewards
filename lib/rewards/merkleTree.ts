...
/**
 * OpenZeppelin sorted-pair Merkle helpers.
 * Must stay byte-compatible with RewardsDistributor + hardhat/test/helpers/merkle.ts.
 */
...
/** Match RewardsDistributor `_claimLeaf` (no campaignId in leaf). */
...
export function buildMerkleTree(leaves: Hex[]): {
  ...
  type Node = { hash: Hex };
  let level: Node[] = leaves.map((hash) => ({ hash }));
